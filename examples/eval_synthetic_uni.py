...
from dripp_uni.experiments.run_multiple_em_on_synthetic import \
    run_multiple_em_on_synthetic
from dripp_uni.config import SAVE_RESULTS_PATH
...
N_JOBS = 3  # number of jobs to run in parallel. To adjust based on machine
...
em_params_to_vary = {'T': np.array([1_000])}
...
for i, this_comb in enumerate(combs):
    ...
    yy = this_comb['baseline'] + this_comb['alpha'] * kernel_true.eval(xx)
    ...
    for seed in range(8):
        ...
        yy = sub_df['baseline_hat'].values[0] + \
            sub_df['alpha_hat'].values[0] * kernel.eval(xx)
...
plt.tight_layout()
plt.savefig(SAVE_RESULTS_PATH / 'fig2_uni.pdf', dpi=300)
plt.show()
plt.close()
