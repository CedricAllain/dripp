...
class DriPP():
    ...
    def __init__(self, lower=30e-3, upper=500e-3, sfreq=150., use_dis=True,
                 init_params=None, initializer='smart_start',
                 alpha_pos=True, n_iter=80, verbose=False, disable_tqdm=False,
                 compute_loss=False):
        self.lower = lower
        self.upper = upper
        self.sfreq = sfreq
    ...
    def fit(self, acti_tt, driver_tt=None, T=None):
        ...
        if acti_tt.dtype == np.float_:
            self.params_, self.history_params_ = em_truncated_norm(
                acti_tt, driver_tt, self.lower, self.upper, T,
                self.sfreq, self.use_dis, self.init_params,
                self.initializer, self.alpha_pos, self.n_iter,
                self.verbose, self.disable_tqdm, self.compute_loss
            )
            baseline, alpha, m, sigma = self.params_
            self.dict_params_ = dict(
                baseline=baseline, alpha=alpha, m=m, sigma=sigma)
    ...
    def cost(self, acti_tt, driver_tt=None, T=None):
        ...
        kernel = [TruncNormKernel(
            self.lower, self.upper, self.sfreq, self.use_dis)
            for _ in range(n_drivers)]
        intensity = Intensity(
            kernel=kernel, driver_tt=driver_tt, acti_tt=acti_tt)
