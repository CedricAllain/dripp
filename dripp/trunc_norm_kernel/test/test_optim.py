...
def test_initialize_baseline():
    ...
    kernel = TruncNormKernel(lower, upper)
    intensity = Intensity(kernel, driver_tt=driver_tt, acti_tt=acti_tt)

    assert initialize_baseline(
        intensity, T) == true_baseline_init
    # %%
...
def test_initialize():
    ...
    _, alpha_init, m_init, sigma_init = initialize(
        intensity, T, initializer='smart_start', seed=None)
    true_alpha_init = [1, 1]
